...
void ComportamientoJugador::procesar_punto_referencia(int fila, int columna,
  ...
  proyectar_mapa_temporal(pos_inicial, pos_inicial, actual.fila,
                          actual.columna);
}
...
Action ComportamientoJugador::think(Sensores sensores) {
  ...
  static const int TAM_SENSOR = 12;
  ...
  if (pk_encontrado == false) {
    if (terreno[0] == 'K') {
      ...
      procesar_punto_referencia(sensores.mensajeF, sensores.mensajeC,
                                POS_POR_DEFECTO);
      actual_valida = true;
      pk_encontrado = true;
      actual.fila = sensores.mensajeF;
      ...
      if (!alcanzando_pk && pk != terreno.end()) {
        auto pk_pos =
            proyectar_vector(actual.orientacion, pk - terreno.begin());
        destino.fila = pk_pos.first;
        destino.columna = pk_pos.second;
        plan.clear();
        pathFinding(actual, destino, plan);
        cout << "Generando camino hasta PK: " << pk - terreno.begin() 
  ...
  // Si se cambia el objetivo, crear un nuevo plan
  if (pk_encontrado && (destino.fila != sensores.destinoF ||
                        destino.columna != sensores.destinoC)) {
    destino.fila = sensores.destinoF;
    destino.columna = sensores.destinoC;
    
    cout << "Asigado destino: " << destino.fila << ' ' << destino.columna << endl;
  ...
  if (posible_colision) {
    ...
    if (pk_encontrado || alcanzando_pk) {
      if (aldeano) {
